...
module.exports.run = async (bot, message, args) => {
    ...
    message.guild.createChannel(userName + "-" + userDiscriminator, "text").then((createdChan) => { // Maak kanaal
        ...
        createdChan.setParent(categoryId).then((settedParent) => { // Zet kanaal in category.
            ...
            settedParent.overwritePermissions(message.guild.roles.find('name', "@👨🏼Inwoner"), { "READ_MESSAGES": false });
            settedParent.overwritePermissions(message.guild.roles.find('name', "@everone"), { "READ_MESSAGES": false });
